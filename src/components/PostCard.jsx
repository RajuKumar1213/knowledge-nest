...
function PostCard({
  ...
  return (
    <Link to={`/post/${$id}`}>
      <div className="z-100 relative w-full overflow-hidden rounded-lg shadow-lg border-gray-500 border p-4">
        {/* Image Section */}
        <img
          src={
            featuredImage ? appwriteStorage.getFilePreview(featuredImage) : ""
          }
          alt={title}
          className="rounded-lg w-full h-64 object-cover transition-transform duration-300 transform hover:scale-95 shadow-lg"
        />

        <h2 className="pt-4  text-lg md:text-xl font-bold text-white">
